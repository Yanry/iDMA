...
module axi_addr_offset_sv #(
    ...
    // AW Channel
    output logic [AxiIdWidth-1:0]     axi_mst_req_aw_id,
    ...
    // B Channel
    input  logic [AxiIdWidth-1:0]     axi_mst_rsp_b_id,
    ...
    // AR Channel
    output logic [AxiIdWidth-1:0]     axi_mst_req_ar_id,
    ...
    // R Channel
    input  logic [AxiIdWidth-1:0]     axi_mst_rsp_r_id,
    ...
    // AXI Slave Interface
    // AW Channel
    input  logic [AxiIdWidth-1:0]     axi_slv_req_aw_id,
    ...
    // B Channel
    output logic [AxiIdWidth-1:0]     axi_slv_rsp_b_id,
    ...
    // AR Channel
    input  logic [AxiIdWidth-1:0]     axi_slv_req_ar_id,
    ...
    // R Channel
    output logic [AxiIdWidth-1:0]     axi_slv_rsp_r_id,
    ...
    // Addr rule
    // input  logic [$clog2(NoMstPorts)-1:0]   rule_idx,
    input  logic [AxiAddrWidth-1:0]   rule_start_addr,
    input  logic [AxiAddrWidth-1:0]   rule_end_addr
    ...
    `AXI_TYPEDEF_AW_CHAN_T(axi_slv_aw_chan_t, addr_t, axi_id_t, user_t)
    `AXI_TYPEDEF_W_CHAN_T(axi_slv_w_chan_t, data_t, strb_t, user_t)
    `AXI_TYPEDEF_B_CHAN_T(axi_slv_b_chan_t, axi_id_t, user_t)
    `AXI_TYPEDEF_AR_CHAN_T(axi_slv_ar_chan_t, addr_t, axi_id_t, user_t)
    `AXI_TYPEDEF_R_CHAN_T(axi_slv_r_chan_t, data_t, axi_id_t, user_t)
    ...
    axi_pkg::xbar_rule_64_t addr_map;
    // Addr rule
    assign addr_map.idx = 0;
    assign addr_map.start_addr = rule_start_addr;
    assign addr_map.end_addr   = rule_end_addr;
    ...
    axi_addr_offset #(
    	...
    	.slv_rsp_o (axi_slv_rsp),
    	.mst_req_o (axi_mst_req),
    	.mst_rsp_i (axi_mst_rsp),
    	.addr_map_i(addr_map)
    );

endmodule : axi_addr_offset_sv