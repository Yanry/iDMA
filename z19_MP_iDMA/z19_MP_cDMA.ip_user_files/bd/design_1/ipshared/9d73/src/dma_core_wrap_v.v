module dma_core_wrap_v #(
    ...
    parameter NumAxInFlight    = 32'd3,
    parameter MemSysDepth      = 32'd0,
    parameter JobFifoDepth     = 32'd0,
    parameter RAWCouplingAvail = 1'b1,
    parameter IsTwoD           = 1'b0
)(
    // Clock and Reset
    input  wire                               clk,
    input  wire                               rst_n,
    ...
    // AW Channel
    output wire [AxiIdWidth-1:0]             axi_mst_req_aw_id,
    ...
    // B Channel
    input  wire [AxiIdWidth-1:0]             axi_mst_rsp_b_id,
    ...
    // AR Channel
    output wire [AxiIdWidth-1:0]             axi_mst_req_ar_id,
    ...
    // R Channel
    input  wire [AxiIdWidth-1:0]             axi_mst_rsp_r_id,
    ...
    // AXI Slave Interface
    // AW Channel
    input  wire [AxiSlvIdWidth-1:0]          axi_slv_req_aw_id,
    ...
    // B Channel
    output wire [AxiSlvIdWidth-1:0]          axi_slv_rsp_b_id,
    ...
    // AR Channel  
    input  wire [AxiSlvIdWidth-1:0]          axi_slv_req_ar_id,
    ...
    // R Channel
    output wire [AxiSlvIdWidth-1:0]          axi_slv_rsp_r_id,
    ...
    dma_core_wrap_sv #(
        .AxiAddrWidth     ( AxiAddrWidth       ),     // 64-bit address width
        .AxiDataWidth     ( AxiDataWidth       ),     // 64-bit data width
        .AxiIdWidth       ( AxiIdWidth         ),     // 1-bit ID width
        .AxiUserWidth     ( AxiUserWidth       ),     // 1-bit user width
        .AxiSlvIdWidth    ( AxiSlvIdWidth      ),     // 1-bit slave ID width
        .NumAxInFlight    ( NumAxInFlight      ),     // Support 3 outstanding transactions
        .MemSysDepth      ( MemSysDepth        ),     // Memory system depth
        .JobFifoDepth     ( JobFifoDepth       ),     // Job FIFO depth
        .RAWCouplingAvail ( RAWCouplingAvail   ),     // RAW coupling disabled
        .IsTwoD           ( IsTwoD             )      // 1D DMA mode
    ) i_dma_core_wrap_sv (
        // Clock and Reset
        .clk                    ( clk                    ),
        .rst_n                 ( rst_n                 ),
